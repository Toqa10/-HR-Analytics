...
def safe_rename_id(df: pd.DataFrame) -> pd.DataFrame:
    """Always expose employee_id even if original column is id."""
...
with d1:
    ...
    if {'age','dept_name','employee_id'}.issubset(snapshot.columns):
        tmp = snapshot.copy()
...
with d2:
    ...
    latest_sal = latest_per_emp(salary, 'from_date') if {'employee_id','amount'}.issubset(safe_rename_id(salary).columns) else None

    # 1 Avg salary per year
    sal_cols = safe_rename_id(salary)
    if {'employee_id','amount','from_date'}.issubset(sal_cols.columns):
        s = sal_cols.copy(); s['from_date'] = to_dt(s['from_date']); s['year'] = s['from_date'].dt.year
    ...
    if {'employee_id','amount'}.issubset(sal_cols.columns):
        top = sal_cols.groupby('employee_id')['amount'].max().sort_values(ascending=False).head(20).reset_index()
        top.columns = ['Employee ID','Top Salary']
        # Streamlit يقبل DataFrame مباشرة
        card("💰 Top 20 Salaries (Table)", None, top,
    ...
    if {'employee_id','amount'}.issubset(sal_cols.columns):
        g = sal_cols.groupby('employee_id')['amount'].agg(['min','max']).reset_index(); g = g[g['min']>0]
...
with d3:
    ...
    if promos_ready:
        ...
        emp_cols = safe_rename_id(employee)
        if 'hire_date' in emp_cols.columns:
            first_change = tdf[tdf['changed']==1].groupby('employee_id')['from_date'].min().reset_index().rename(columns={'from_date':'first_promo_date'})
            tmp = emp_cols[['employee_id','hire_date']].merge(first_change, on='employee_id', how='inner')
        ...
        st.info("title.csv needs columns: employee_id (or id), title, from_date")

# ======================= RETENTION & TURNOVER (7) ==============
with d4:
    pal = PALETTES['ret']

    # نجهز نسخة employee بأمان
    emp_cols = safe_rename_id(employee)
    ...
    if 'hire_date' in emp_cols.columns:
        if 'termination_date' in emp_cols.columns:
            df = emp_cols[['employee_id','hire_date','termination_date']].copy()
        else:
            df = emp_cols[['employee_id','hire_date']].copy()
        df['hire_year'] = to_dt(df['hire_date']).dt.year
        hires = df['hire_year'].value_counts().sort_index().cumsum().reset_index()
        hires.columns = ['Year','Active+']
    ...
    if {'hire_date','termination_date'}.issubset(emp_cols.columns):
        ...
        # نظهر نفس الكارت بعنوانه مع تنبيه أنه غير متاح لغياب termination_date
        card("🧾 Turnover Rate by Year (Approx)", None,
             desc="Not available: employee.csv has no termination_date. Upload/merge terminations to enable this metric.",
             insights=["Turnover requires termination events to compute."],
             recs=["Add a terminations file or a status column to compute turnover."])

    # 5 Attrition by tenure band (if terminations)
    if {'termination_date','hire_date'}.issubset(emp_cols.columns):
        e = emp_cols[['employee_id','hire_date','termination_date']].dropna(subset=['termination_date']).copy()
        ...
        card("🚪 Attrition by Tenure Band", None,
             desc="Not available without termination_date.",
             insights=["You need exit dates to bucket attrition by tenure."],
             recs=["Provide termination_date to activate this chart."])

    # 6 Attrition by department (if terminations)
    if 'termination_date' in emp_cols.columns and 'dept_name' in snapshot.columns:
        left = emp_cols[['employee_id','termination_date']].dropna().copy()
        ...
        card("🏃 Attrition by Department", None,
             desc="Not available without termination_date.",
             insights=["Department-level exit counts require actual terminations."],
             recs=["Add termination_date to enable department attrition."])

    # 7 Retention by hire cohort (survival proxy)
    if 'hire_date' in emp_cols.columns:
        e = emp_cols[['employee_id','hire_date']].copy()
        # لو termination_date موجود نحسب retention حقيقي؛ لو مش موجود نحسبه كسفلي (Lower-bound)
        if 'termination_date' in emp_cols.columns:
            e['termination_date'] = to_dt(emp_cols['termination_date'])
            e['left'] = ~e['termination_date'].isna()
            e['cohort'] = to_dt(e['hire_date']).dt.year
            e['retained_1y'] = (~e['left']) | ((e['termination_date'] - to_dt(e['hire_date'])).dt.days >= 365)
            note = "Actual 1-year retention by cohort."
        else:
            e['cohort'] = to_dt(e['hire_date']).dt.year
            # بدون termination_date: الموظفون الحاليون فقط؛ اللي مرّ عليهم >= سنة يعتبر Retained (Lower-bound)
            days_in_company = (pd.to_datetime('today') - to_dt(e['hire_date'])).dt.days
            e['retained_1y'] = days_in_company >= 365
            note = "Lower-bound estimate (no termination_date)."
        ...
        card("🛡 1-Year Retention by Hire Cohort", fig,
             desc=f"Share of each hire cohort retained at least 1 year. {note}",
