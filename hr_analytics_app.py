...
# Optional: statsmodels for trendline in plotly
try:
...
def ensure_columns(df, cols):
    for c in cols:
        if c not in df.columns:
            if any(k in c for k in ("date", "hire", "termination", "from_date", "to_date")):
                df[c] = pd.NaT
            else:
                df[c] = np.nan
    return df
...
base_emp = employee.copy() if "employee_id" in employee.columns else (current_snapshot[["employee_id"]].drop_duplicates() if "employee_id" in current_snapshot.columns else pd.DataFrame(columns=["employee_id"]))
...
# sidebar radio - keep synced with session_state
page = st.sidebar.radio("Page", PAGES, index=PAGES.index(st.session_state.page))
st.session_state.page = page

# Header
st.title("HR Analytics Dashboard")
st.markdown("30+ charts across Demographics, Salaries, Promotions, and Retention. Each chart includes Description → Insights → Recommendations.")
st.markdown("---")
...
- Descriptions, insights and recommendations below charts

**Data (recommended)**
- employee.csv (employee_id or id, birth_date, hire_date, termination_date, gender)
...
def render_demographics():
    ...
    if snapshot["age"].notna().any():
        ...
        render_card("Age Distribution", fig,
                    description="Histogram of employee ages.",
                    insights=["Dominant cohorts and outliers."],
                    recs=["Tailor learning & benefits to dominant cohorts."])

    # 2 Age groups by department
    if snapshot[["age","dept_name"]].dropna().shape[0] > 0:
        ...
        if pivot.shape[1] > 1:
            ...
            render_card("Age Group Composition by Department", fig,
    ...
    if snapshot["dept_name"].notna().any():
        ...
        render_card("Headcount by Department", fig,
    ...
    if snapshot["title"].notna().any():
        ...
        render_card("Top Titles by Headcount (Top 20)", fig,
    ...
    if snapshot[["age","dept_name"]].dropna().shape[0] > 0:
        ...
        render_card("Age by Department (Box)", fig,
    ...
    if gender_col and snapshot[gender_col].notna().any():
        ...
        render_card("Gender Mix (Overall)", fig,
    ...
    if gender_col and snapshot[["dept_name", gender_col]].dropna().shape[0] > 0:
        ...
        render_card("Gender Ratio by Department", fig,
    ...
    if snapshot[["age","company_tenure"]].dropna().shape[0] > 0:
        ...
        render_card("Age vs Tenure (Heatmap)", fig,
...
def render_salaries():
    ...
    if {"employee_id","amount","from_date"}.issubset(salary.columns) and salary.shape[0] > 0:
        ...
        render_card("Average Salary Over Time", fig,
    ...
    if {"employee_id","amount"}.issubset(salary.columns) and salary.shape[0] > 0:
        ...
        render_card("Top 20 Salaries (Table)", table=top,
    ...
    if latest_sal.shape[0] > 0:
        ...
        render_card("Salary Distribution (Latest)", fig,
    ...
    if latest_sal.shape[0] > 0 and snapshot["dept_name"].notna().any():
        ...
        if m.shape[0] > 0:
            ...
            render_card("Average Salary by Department", fig,
    ...
    if latest_sal.shape[0] > 0 and snapshot["company_tenure"].notna().any():
        ...
        try:
            ...
            render_card("Tenure vs Salary", fig,
                        description="Relationship between tenure and pay (trendline only if statsmodels installed).",
                        insights=["Reveals pay progression with tenure."],
    ...
    if {"employee_id","amount"}.issubset(salary.columns) and salary.shape[0] > 0:
        ...
        if g.shape[0] > 0:
            ...
            render_card("Top 10 Salary Growth %", fig,
    ...
    if latest_sal.shape[0] > 0 and snapshot["dept_name"].notna().any():
        ...
        if m.shape[0] > 0:
            ...
            render_card("Salary Spread by Department", fig,
    ...
    if latest_sal.shape[0] > 0 and snapshot["title"].notna().any():
        ...
        if m.shape[0] > 0:
            ...
            render_card("Average Salary by Title (Top 30)", fig,
...
def render_promotions():
    ...
    if {"employee_id","title","from_date"}.issubset(title.columns) and title.shape[0] > 0:
        ...
        if per_year.shape[0] > 0:
            ...
            render_card("Promotions per Year", fig,
        ...
        if "hire_date" in employee.columns and employee["hire_date"].notna().any():
            ...
            if emp_tmp.shape[0] > 0:
                ...
                render_card("Time to First Promotion (Years)", fig,
        ...
        if snapshot["dept_name"].notna().any():
            ...
            if by_dept.shape[0] > 0:
                ...
                render_card("Promotions by Department", fig,
        ...
        if top_multi.shape[0] > 0:
            ...
            render_card("Employees with Multiple Promotions (Top 20)", fig,
        ...
        if gender_col:
            ...
            if by_g.shape[0] > 0:
                ...
                render_card("Promotions by Gender", fig,
        ...
        if path_len.shape[0] > 0:
            ...
            render_card("Career Path Length (Title Steps)", fig,
        ...
        if snapshot["dept_name"].notna().any():
            ...
            if heat.shape[0] > 0 and heat.shape[1] > 0:
                ...
                render_card("Promotions Heatmap (Dept × Year)", fig,
...
def render_retention():
    ...
    if snapshot["company_tenure"].notna().any():
        ...
        render_card("Tenure Distribution (Years)", fig,
    ...
    if snapshot[["dept_name","company_tenure"]].dropna().shape[0] > 0:
        ...
        render_card("Tenure by Department (Box)", fig,
    ...
    if "hire_date" in employee.columns and employee["hire_date"].notna().any():
        ...
        if not emp_h.empty:
            ...
            render_card("Active Headcount Over Time (Approx)", fig,
    ...
    if "termination_date" in employee.columns and employee["termination_date"].notna().any():
        ...
        render_card("Terminations per Year", fig,
    ...
    if "termination_date" in employee.columns and snapshot["dept_name"].notna().any():
        ...
        if rate.shape[0] > 0:
            ...
            render_card("Attrition Rate by Department", fig,
    ...
    if {"employee_id","amount"}.issubset(salary.columns) and snapshot["company_tenure"].notna().any():
        ...
        if mm.shape[0] > 0:
            ...
            render_card("Tenure vs Salary (Heatmap)", fig,
    ...
    if "employee_id" in department_employee.columns:
        ...
        if moves.shape[0] > 0:
            ...
            render_card("Department Moves per Employee", fig,
...
# ---------------- Sync query params (new API usage) ----------------
# set simple page query param so URL reflects current page
try:
    # Use list values to be compatible with query param patterns
    st.query_params = {"page": [st.session_state.page]}
except Exception:
    # If assignment fails for environment, ignore silently
    pass
