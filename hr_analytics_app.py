...
def load_data():
    ...
    if {"employee_id","dept_id"}.issubset(dept_emp.columns) and {"dept_id","dept_name"}.issubset(dept.columns):
        d_latest = latest_per_emp(dept_emp, "to_date" if "to_date" in dept_emp.columns else "from_date").merge(dept, on="dept_id", how="left")[["employee_id","dept_name"]]
    else:
...
tab_choice = st.sidebar.radio("Select Dashboard Section:", [
    "👤 Demographics",
...
if tab_choice == "👤 Demographics":
    ...
    # باقي الرسوميات من كودك داخل هذا block ...

# ==================== SALARIES & COMPENSATION ==================
elif tab_choice == "💵 Salaries & Compensation":
    pal = PALETTES['pay']
    # كل الرسوميات الخاصة بالرواتب هنا (نفس الكود الموجود في قسمك d2)

# ================= PROMOTIONS & CAREER GROWTH ==================
elif tab_choice == "🚀 Promotions & Career Growth":
    pal = PALETTES['promo']
    # كل الرسوميات الخاصة بالترقيات ومسار الوظائف (كودك في d3)

# ======================= RETENTION & TURNOVER ==================
elif tab_choice == "🧲 Retention & Turnover":
    pal = PALETTES['ret']
    # كل الرسوميات الخاصة بالاحتفاظ والترك (كودك في d4)
